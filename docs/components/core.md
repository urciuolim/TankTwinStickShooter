...
- [`core.maps`](../../src/pop_trainer/core/maps.py) — the **map-rotation resolution contract**
  ...
  arena-target strings (`Arenas/<name>.json`, alphabetical by arena name, `maps.py:41-52`) — there
  is **no on-disk config-wrapper dir** anymore. [`resolve_map_rotation(values)`](../../src/pop_trainer/core/maps.py)
  (`maps.py:71-86`) turns a `--maps` flag value into a `list[str]` of arena targets (or `None`):
  `None` (flag absent) → `None` (single-map mode); `[]` or `[ALL_MAPS_SENTINEL]` (flag, no value) →
  `list(CURATED_ROTATION)`; a single **directory** entry → each `*.json` config's `arena_path`
  sorted by filename (a dir of map-config wrappers still works as a rotation source); an explicit
  **list** → its arena targets verbatim, in the given order. The arena GEOMETRY those targets name
  is (re)generated by `exp-configs/_generate_maps.py` (see [experiments](../experiments.md)); this
  constant is just the curation of which arenas the rotation plays.
