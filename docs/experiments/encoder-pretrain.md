...
- **Status:** harness built + GPU-validated; **flat-val hit twice** — first the position-blind GAP heads (fixed by the soft-argmax decoder), then the soft-argmax decoder's own **predict-center basin** (coord-only escape gradient too weak; capacity fix insufficient, §6) → **heatmap-CE fix landed + validated** (§6/§10): spike-validated (~0.4–0.5 wu), then production was flat until the **shard-grouped sampler** (single-episode, no-diversity batches) was found to starve it — with cross-shard batch diversity (validated by a one-off on-disk reshuffle) val drops **4.06 → 0.40 wu in 3 epochs**. **Sweep 1 (3 of 10 cells) now landed** (§2 → Experimental log): the fix holds at full training scale (~0.30 wu), **`resnet × flatten @ 180` leads**, with **aim (~41°) + presence (F1 ~0.46)** the architecture-independent bottlenecks. The **windowed multi-shard sampler** is now committed + **learning-validated on the pristine `decode-v2_original`** (val 2.15 → 0.31 wu) — but it's **NO-GO on perf** (per-worker cache replication at `num_workers=8` → ~36 GB / ~9× slow); the **reproducible dataset-shuffle tool (Option C) has since landed** ([#14](https://github.com/urciuolim/TankTwinStickShooter/issues/14), merged `0f926a0` — `pop_trainer.data.shuffle`), so the runtime sampler-perf rework is moot; the sweep meanwhile runs on the diverse `decode-v2_reshuffled` at `--shard-window 1`. **Now:** **Exp #3 readout fixes** running — aim up-weight × heatmap-sharpen × resolution on the leader (8-cell knob sweep, §2 #3) to close the aim (~41°) / bullet-pos (~1.3 wu) / presence (calibration) gaps Sweep 1 exposed; then the resolution axis (#4). Tracked at [#15](https://github.com/urciuolim/TankTwinStickShooter/issues/15).
- **Owner:** Brian (CTO-delegated eval/encoder direction).
- **Tracking:** [#3](https://github.com/urciuolim/TankTwinStickShooter/issues/3) (parent) · [#5–8](https://github.com/urciuolim/TankTwinStickShooter/issues/5) (phases) · [#12](https://github.com/urciuolim/TankTwinStickShooter/issues/12) (follow-up) · **[#15](https://github.com/urciuolim/TankTwinStickShooter/issues/15) (experimental program: Sweep 1 → Exp #3/#4/#5)** · [#13](https://github.com/urciuolim/TankTwinStickShooter/issues/13) (model store/card) · [#14](https://github.com/urciuolim/TankTwinStickShooter/issues/14) (reproducible dataset shuffle — ✅ landed) · [#16](https://github.com/urciuolim/TankTwinStickShooter/issues/16) (config-file params). **Code:** PR #11. **Plan:** PR #4.
...
|---|---|---|---|---|---|---|---|---|
| cnn × gap | 360 |  |  |  |  |  |  |  |
| cnn × gap | 180 | 0.137M | — | 4.205 / 12.833 | 85.0 / 84.8 | 1.717 / 3.468 | 0.143 / 0.230 | 5.191 / 12.533 |
| cnn × flatten | 360 |  |  |  |  |  |  |  |
| cnn × flatten | 180 |  |  |  |  |  |  |  |
| resnet × gap | 360 |  |  |  |  |  |  |  |
| resnet × gap | 180 | 0.100M | — | 0.302 / 4.050 | 51.6 / 55.9 | 0.772 / 0.871 | 0.364 / 0.329 | 1.453 / 4.100 |
| resnet × gap | 90 |  |  |  |  |  |  |  |
| resnet × flatten | 360 |  |  |  |  |  |  |  |
| resnet × flatten | 180 | 0.100M | — | 0.326 / 0.539 | 41.3 / 41.2 | 0.757 / 0.762 | 0.386 / 0.464 | 1.322 / 2.043 |
| resnet × flatten | 90 |  |  |  |  |  |  |  |

*(`cnn@90` dropped — CnnTrunk's ÷4 stem underflows a 90-row frame.)*
...
|---|---|---|---|
| 1 | **Sweep 1** — trunk × pooling @180 | ✅ Done (2026-06-28) | Leader = `resnet × flatten @ 180`; aim (~41°) & presence (F1 ~0.46) are architecture-independent bottlenecks |
...
| 3 | **Readout fixes on the leader** — aim up-weight × heatmap-sharpen × resolution, on `resnet × flatten` | 🔄 In progress (2026-06-29) | Decloak the true metric ceiling: close aim/bullet-pos gaps + presence recalibration; 8-cell knob sweep running |
| 4 | **Resolution axis (gated)** — `resnet × flatten @360` (+ `resnet × gap @360` confirm), multi-seed winner | 📋 Planned · conditional on #3 | Test the one config (resolution) that can move aim/bullet-pos; lock the winner |
| 5 | **Principal cut + encoder→RL transfer probe** | 📋 Planned · high VOI | Validate the decode proxy against real RL usefulness (§7.1) |

#### 1 · Sweep 1 — trunk (resnet vs cnn) × pooling (gap vs flatten) at 180 — ✅ done (2026-06-28)

**What ran.** The first three of the ten cells, chosen to settle the two architecture axes (trunk, pooling) at a single mid resolution before spending compute on the resolution axis — and to confirm the diverse-batch sampler fix holds on full production runs (not just the one-off reshuffle spike, §6/§10). Cells: `resnet × gap`, `resnet × flatten`, `cnn × gap`, all at 180 (320×180). Config: 12 epochs, batch 256, lr 3e-3, seed 0, `datasets/decode-v2_reshuffled` (70.8k train / 22.7k val / 26.4k test), heatmap-CE decoder (`heatmap_weight=6.0`, `heatmap_sigma=0.7`), on the local RTX 4090 (`--device cuda`). All three completed clean — no NaNs/errors. Runs: `runs/pc-{resnet-gap,resnet-flatten,cnn-gap}-180`.

**Results** (test set, `spatial / embed-probe`; see §2 table for the full row):

| Cell | Position (wu) | Aim (°) | Presence F1 | Bullet-pos (wu) |
|---|---|---|---|---|
| resnet × gap | **0.302** / 4.050 | 51.6 / 55.9 | 0.364 / 0.329 | 1.453 / 4.100 |
| resnet × flatten | 0.326 / **0.539** | **41.3** / **41.2** | **0.386** / **0.464** | **1.322** / **2.043** |
| cnn × gap | 4.205 / 12.833 | 85.0 / 84.8 | 0.143 / 0.230 | 5.191 / 12.533 |

**Gap to target & how to close it.** Bands from §1.1 (🟡 Good / 🟢 Excellent). "Leader best" = `resnet × flatten @ 180` (the chosen direction), as `spatial / probe`. Gap is measured against 🟡 Good on the **embed-probe** (the RL-relevant artifact) unless noted.
...
1. **Sampler fix confirmed in production.** Both resnet cells land at ~0.3 wu spatial position — matching the reshuffle spike's 4.06 → ~0.40 wu prediction. No regression to the old flat-val basin. The diverse-batch productionization holds on full runs.
2. **resnet ≫ cnn, decisively.** `cnn × gap` never escaped the predict-the-center basin (4.2 wu position, 85° aim, presence F1 0.14) — CnnTrunk's shallow trunk lacks the capacity to localize here. resnet is the trunk going forward.
...
**Leader so far:** `resnet × flatten @ 180` — best embed-probe retention, which is the property the downstream RL encoder needs.
...
|---|---|---|---|---|---|---|---|---|
| **resnet × flatten (leader)** | **spatial** | **0.790** | 0.986 | 0.483 | 0.92 | **0.746** | 0.74 / 0.76 | +0.263 |
| **resnet × flatten (leader)** | **probe** | **0.778** | 0.985 | 0.491 | 0.87 | **0.730** | 0.69 / 0.78 | +0.239 |
| resnet × gap | spatial | 0.794 | 0.983 | 0.425 | 0.94 | 0.743 | 0.77 / 0.71 | +0.317 |
| resnet × gap | probe | 0.700 | 0.982 | 0.359 | 0.93 | 0.694 | 0.66 / 0.73 | +0.336 |
| cnn × gap | spatial | 0.135 | 0.896 | 0.186 | 0.01 | 0.193 | 0.13 / 0.40 | +0.007 |
| cnn × gap | probe | 0.171 | 0.916 | 0.264 | 0.01 | 0.264 | 0.15 / 0.94 | +0.000 |

**Verdict — calibration-limited (leader, decisively).** PR-AUC ≈ 0.79 against a 3.26% base rate is **~24× chance** (chance AP ≈ 0.033); ROC-AUC ≈ 0.99 — the encoder *already separates present/absent well at 180*. Pure recalibration lifts leader F1 **0.49 → 0.73–0.75, clearing the §1.1 "Good" band (≥0.70) with no retraining**. The F1-optimal threshold sits far out at **~0.87–0.92** (logit ≈ +2 to +2.4), not 0.5 — the textbook fingerprint of `pos_weight≈28.9` inflating positive logits to buy recall (R≈0.93/P≈0.33 at 0.5) at precision's cost. Both resnet cells are calibration-limited; `cnn × gap` is *discrimination*-limited (PR-AUC ~0.15 ≈ chance, thresholding can't help) — but that only re-confirms the dead trunk, it doesn't affect the decision.
...
|---|---|---|---|---|---|---|
| **resnet × flatten (leader)** | **spatial** | 0.92 | 0.802 | **0.762** | 0.763 | +0.0002 |
| **resnet × flatten (leader)** | **probe** | 0.87 | 0.799 | **0.739** | 0.739 | +0.0007 |
| resnet × gap | spatial | 0.94 | 0.792 | 0.744 | 0.746 | +0.0023 |
| resnet × gap | probe | 0.93 | 0.677 | 0.681 | 0.682 | +0.0009 |

The leader clears the §1.1 "Good" band (≥0.70) on truly held-out test in **both** families (spatial 0.762, probe 0.739). `resnet × gap` probe lands just under (0.681), consistent with its weaker PR-AUC (0.68 — partly discrimination-bound). Scratch script: `.scratch/presence_threshold_transfer.py`.
...
#### 3 · Readout fixes on the leader — 🔄 in progress (2026-06-29)

**Goal.** Decloak the true metric ceiling of the leader (`resnet × flatten`) by fixing the three architecture-independent readout problems Sweep 1 (#1) exposed — aim (~41°), bullet-pos (~1.32 wu), presence (F1 ~0.39 @ thr-0). Sweep 1 showed these don't move with trunk/pooling, so they're attacked at the **readout/loss level** on the single leading config, *before* committing compute to a wider grid (§4 one-factor attribution — don't confound a readout fix with an architecture change).

**Sampler note.** No longer gated on the loader perf rework: `decode-v2_reshuffled` already mixes all 10 maps into every shard, so this runs today at `--shard-window 1 --num-workers 8` (~6.4 GB) on the committed pipeline. (The reproducible way to produce such a dataset — Option C — has since landed as [#14](https://github.com/urciuolim/TankTwinStickShooter/issues/14).)

**What changed vs Sweep 1 (Exp #1).** Same trunk/pooling/data lineage (`resnet × flatten`, `decode-v2_reshuffled`, 12 epochs, batch 256, lr 3e-3, seed 0, `heatmap_weight=6.0`, CUDA). The deltas are the readout knobs added in build commit `c856005` (platform-gate GO, 111 tests):

| Lever | Sweep 1 | Exp #3 | Mechanism |
|---|---|---|---|
| **Aim weight** | 1.0 (unit) | **swept {5, 10}** (`--group-weights player_aim=`) | Aim's `1−cos` term was swamped by the heatmap-CE term (weight 6.0) → tiny gradient share. Up-weighting restores it. (Aim is *already* a `(ax,ay)` direction target + angular `1−cos` loss — **no reparam needed**, contrary to the original #3 sketch.) |
| **Heatmap σ** | 0.7 cells | **swept {0.4, 0.5}** (`--heatmap-sigma`) | Sharper target Gaussian → crisper soft-argmax peak for the tiny (1–2 px) bullets. |
| **Resolution** | 180 only | **swept {180, 360}** (`--resolution`) | 360 halves the world-units/cell of the feature grid — the one lever with a mechanistic reason to move bullet-pos (and the small turret cue for aim). |
| **Presence threshold** | fixed 0.0 | **val-selected, applied to test** (automatic, every run) | New `presence_calibration` block in `results.json`: the threshold maximizing val-F1 is chosen on val and applied *fixed* to held-out test. Exp #2 proved this lifts F1 ~0.39 → ~0.75 with no retrain. `pos_weight` left auto-fit (~28.9) and **collapsed out of the grid** — calibration dominates it. |

**Sweep axes (8 cells = 2 × 2 × 2).** Each axis targets a distinct bottleneck; presence is fixed by the calibration block independently of the grid.

| Axis | Values | Primary target |
|---|---|---|
| aim-weight | 5, 10 | aim (°) |
| heatmap-σ | 0.4, 0.5 | bullet-pos (wu) |
| resolution | 180, 360 | bullet-pos + aim |

Run dirs: `runs/exp3-aim{5,10}-sig{0.4,0.5}-res{180,360}/`. Driver runs the four res-180 cells first, then res-360 (batch-256 OOM risk isolated last; a failed cell is logged and the sweep continues so 180 results are preserved).

**Results.** ⏳ *Sweep running (launched 2026-06-29) — fills in on completion.* Per-cell **test** metrics (`spatial / probe`); presence reported both @thr-0 (legacy) and @val-selected-threshold.

| Cell (aim · σ · res) | Position (wu) | Aim (°) | Bullet-pos (wu) | Presence F1 @thr0 | Presence F1 @val-thr |
|---|---|---|---|---|---|
| aim5 · σ0.4 · res180 |  |  |  |  |  |
| aim5 · σ0.5 · res180 |  |  |  |  |  |
| aim10 · σ0.4 · res180 |  |  |  |  |  |
...
*Baselines to beat — Sweep 1 leader `resnet × flatten @ 180` (spatial): position **0.33 wu**, aim **41.3°**, bullet-pos **1.32 wu**, presence F1 **0.39** @thr0 (→ **~0.76** achievable by calibration, Exp #2).*

**Best cell vs §1.1 bands.** ⏳ *Filled after the sweep.*

| Metric | 🟡 Good / 🟢 Excellent | Best Exp #3 cell | vs Sweep 1 | Verdict |
|---|---|---|---|---|
| Position (wu) | ≤0.30 / ≤0.10 |  |  |  |
| Aim (°) | ≤10 / ≤3–5 |  |  |  |
| Bullet-pos (wu) | ≤0.50 / ≤0.30 |  |  |  |
| Presence (F1) | ≥0.70 / ≥0.85 |  |  |  |

**Gate / what we learn.** At the best cell: does **loss-balance alone** fix aim (→ ≤10°) or does it need a dedicated angular head? Is **res-360 worth the compute** for bullet-pos (→ ≤0.50)? Position must hold ≤0.30 and presence should clear ≥0.70 via calibration. The answers feed the #4 resolution decision and tell us whether aim is a remaining open problem for #15.

**Spike (pre-commit direction check — 3 epochs, aim10 · σ0.4 · res180).** All axes trended correctly before the full sweep committed: aim **77.8 → 55.9°** (↓), bullet-pos **4.00 → 2.26 wu** (↓, escaping the center basin), position **0.76 → 0.44 wu** (↓); calibration path live end-to-end — test presence F1 **0.241 (thr-0) → 0.568 (val-thr 2.41)** at just 3 epochs. Confirmed GO-to-commit (`c856005`).

#### 4 · Resolution axis (gated) — 📋 planned (conditional on #3)

**Goal.** Test the one config with a mechanistic reason to move aim/bullet-pos — resolution — and lock the winner. **What runs:** `resnet × flatten @ 360` (and `@ 90` for the Pareto point); `resnet × gap @ 360` to confirm the flatten≫gap embed-probe gap isn't resolution-specific; **≥3 seeds** on the eventual winner (§4). **Gate:** only run the cells #3 shows are still short of band; the **`cnn` rows are likely droppable** (Sweep 1 shows cnn dominated) — fill only for a complete-grid writeup. **Decision:** pick the accuracy↔compute Pareto cell that clears the bar (§1 decision rule).
...
- **Encoder** (`models.build_encoder`): a CnnTrunk / ResNetTrunk **trunk** × GAP / Flatten **pooling**, with an early ÷4 **stem** sized for the 640×360 native frame.
- **Two head families:**
  - **Spatial heads** read `Encoder.features` (the `(B,C,h,w)` map) → gradient trains the encoder (the reusable artifact). **Readout redesign landed + production-validated** (§6/§10): the original GAP-then-FC readout was position-blind → now a heatmap/soft-argmax localization decoder with auxiliary cross-entropy supervision.
...
- **Data:** shard-streaming `DecodeDataset`, map-aware 80/10/10 split (no leak), **windowed multi-shard sampler** (`ShardWindowBatchSampler` — round-robin across W shards for cross-shard batch diversity; "decompress once/epoch" holds at `num_workers=0`, per-worker cache replication at `num_workers>0` made that path slow — sidestepped by the Option C collection-time shuffle that landed as [#14](https://github.com/urciuolim/TankTwinStickShooter/issues/14), so `--shard-window 1` now suffices), load-time downsample.
...
- **Grid:** `{cnn, resnet} × {gap, flatten} × {360, 180, 90}` = 12 → **10 cells** (`cnn@90` dropped — CnnTrunk's ÷4 stem underflows a 90-row frame).
...
- **Protocol to lock:** seed(s) **[OPEN — ≥3 for the final comparison]**, epochs/cell **= 12** (settled post-fix; converges within 12 at batch 256 / lr 3e-3), LR / batch / normalization fixed (note: scale LR with batch size — `3e-4` was tuned for batch 32), map-aware split.
...
| Epochs / cell = **12** | converges within 12 at batch 256 / lr 3e-3 after the heatmap-CE fix (Sweep 1 + Exp #3) | Brian | ✅ |
...
| 2026-06-27 | **GPU validated (4090, `torch 2.9.1+cu130`)** | `resnet/180` ~20s/epoch; **~2,200 samples/sec** at `num_workers=8` / `batch=256` (~17× the default). Windows spawn works. |
...
| 2026-06-28 | **✅ Sweep 1 — trunk + pooling decided; full-run confirms the reshuffle win** | Ran the first **3 of 10 cells** (`resnet × {gap, flatten}`, `cnn × gap` @180, full 12 epochs, batch 256, lr 3e-3, seed 0, on `decode-v2_reshuffled`, local 4090). Three findings (full writeup + per-cell numbers in §2 → Experimental log → **Sweep 1**; target-band gap analysis added at **§1.1**): **(1)** the diverse-batch fix **holds at full training scale** — both resnet cells reach val/test position **~0.30 wu** (spatial), no regression to the flat basin (matches the reshuffle preview). **(2) resnet ≫ cnn** — cnn never left the predict-center basin (4.2 wu / 85° / F1 0.14); resnet is the trunk. **(3) flatten ≫ gap on the embed-probe** (the RL-relevant artifact): **0.54 vs 4.05 wu** position, F1 0.46 vs 0.33 — gap's spatial head only looks competitive because it reads the *pre-pooled* map; once pooled, GAP averages position away. Cost: flatten decoder ~3.6M vs gap ~1.2M (encoder identical 0.10M). **Leader = `resnet × flatten @ 180`.** Remaining bottlenecks are **architecture-independent**: aim ~41° (≫ ≤10° bar) and presence F1 ~0.46 (< 0.70 bar) cap every cell. §2 rows filled. |
| 2026-06-28 | **⚠️ Sampler productionized + validated on pristine data — LEARNING ✅ / PERF ❌ (NO-GO)** | Committed the windowed multi-shard sampler (`ShardWindowBatchSampler` + per-worker `_LruShardCache` capacity W+1; commit `906a974`; **sampler unit tests 7/7 pass**). Validated it through the committed pipeline on the **pristine `decode-v2_original`** (`resnet × flatten @ 180`, 12 epochs, `--num-workers 8 --shard-window 8 --device cuda`; `runs/sampler-prod-validate`). **LEARNING ✅:** val position **2.15 → 0.31 wu** over 12 epochs (test pos **0.30**, aim 37.5°, presence-F1 0.55, bullet-pos 1.39) — the diverse-batch fix reproduces end-to-end on the committed sampler with **no manual reshuffle and no flat basin** → closes To-do #1's pristine-data validation. **PERF ❌ NO-GO:** **~2:55/epoch (~9× Sweep 1's ~20s)** at **~36 GB RAM**, GPU pinned at **0%** — root cause: with `num_workers=8` the LRU cache is **replicated per DataLoader worker** (8× copies + each shard decompressed ~8×, not once/epoch), so the "decompress once" guarantee only holds at `num_workers=0`. Verdict: **correctness GO, perf NO-GO** → loader rework planned (eng-manager + tech-lead, spike-gated: threaded-prefetch + GPU-side transform **vs** uncompressed-`.npy` + memmap so the OS page cache is the shared cross-process cache; GO bar < 60s/epoch + bounded RAM + learning preserved). |
| 2026-06-29 | **🔄 Exp #3 readout fixes — unblocked, built, sweep running** | Found the sampler perf NO-GO **no longer gates progress**: `decode-v2_reshuffled` already mixes all 10 maps into every shard, so Exp #3 runs at `--shard-window 1 --num-workers 8` (~6.4 GB) → loader rework no longer gating; the Option C reproducible-shuffle tool then landed via [#14](https://github.com/urciuolim/TankTwinStickShooter/issues/14) (merged `0f926a0`, `pop_trainer.data.shuffle`). Built the readout-fix knobs (commit `c856005`, platform-gate GO, **111 tests**): `--group-weights` (aim up-weight), `--presence-pos-weight` override, and a **val-selected presence threshold** reported on val+test (`presence_calibration` block). 3-epoch spike trended correctly on every axis (aim 77.8→55.9°, bullet-pos 4.0→2.26 wu, calibrated presence F1 0.24→0.57); launched the **8-cell knob sweep** (aim-weight {5,10} × σ {0.4,0.5} × resolution {180,360}) on the leader `resnet × flatten`. Experimental program tracked at [#15](https://github.com/urciuolim/TankTwinStickShooter/issues/15); config-file-params cleanup filed as [#16](https://github.com/urciuolim/TankTwinStickShooter/issues/16). §2 #3 scaffolded; fills on completion. |

### To do
Open next steps + decisions (was §7's "next steps / open questions") — all **📋 to do** unless flagged otherwise.

1. 🟢 **Diverse-batch fix — code done, learning-validated; perf NO-GO but no longer blocking.** The windowed multi-shard sampler is committed (`906a974`, `ShardWindowBatchSampler` + LRU shard cache) and ✅ **validated on the pristine `decode-v2_original`** (val **2.15 → 0.31 wu**, §6 2026-06-28). **Perf is still NO-GO** at `num_workers=8` (LRU cache replicates per DataLoader worker → **~36 GB RAM + ~9× slower**, since "decompress once/epoch" only holds at `num_workers=0`) — **but this no longer blocks the sweep:** `decode-v2_reshuffled` already supplies cross-shard diversity, so Exp #3+ run at `--shard-window 1 --num-workers 8` (~6.4 GB). The loader rework **✅ landed as Option C** ([#14](https://github.com/urciuolim/TankTwinStickShooter/issues/14), merged `0f926a0`): `pop_trainer.data.shuffle` — a reproducible, provenanced, memory-bounded two-pass out-of-core shuffle with both `global` (C1) and `within-split` (C2) modes (reviewed GO, 156 passed), chosen over the runtime Finalist-A/B machinery (B dropped ~140× disk; A shelved). **Remaining:** C1-vs-C2 is **settled → `global` (C1) is the chosen default** (the `--split-mode` flag still carries `within-split`/C2 for later). The only step left is to *run* the tool to materialize a canonical, seeded dataset (`decode-v2_shuffled-s0`, replacing the throwaway unknown-seed `decode-v2_reshuffled`) and point the sweeps at it — **not yet run** (non-urgent; the sweep uses the existing reshuffled data). **Still open:** (c) heads-up to Mike on PR #11 re: the decoder-approach change + the sampler finding. *(Validation artifacts: `datasets/decode-v2_reshuffled`, `datasets/decode-v2_original`, `runs/sampler-prod-validate` — all git-ignored, local only.)*
...
5. 🟠 **Continue the sweep — resolution axis for the leader** (next batch, "Sweep 2"). From Sweep 1's leader `resnet × flatten @ 180`: (a) run `resnet × flatten @ {360, 90}` — is 180 on the accuracy↔compute Pareto frontier, or does 360 buy enough to justify the cost? (b) `resnet × gap @ 360` to confirm the flatten≫gap embed-probe gap isn't resolution-specific; (c) **multi-seed the eventual winner** (≥3 seeds, per §4) before locking the decision. The `cnn` rows are likely droppable — Sweep 1 shows cnn dominated — fill them only if a complete trunk comparison is wanted for the writeup.
6. 🟠 **Close the per-metric gaps** (architecture-independent; won't move with trunk/pooling — see the §2 Sweep 1 gap table + bands at §1.1).
   - **Aim (~41° vs ≤10° bar) — biggest lever; being tested in Exp #3.** Aim is *already* a `(ax, ay)` unit-direction target trained by an angular `1−cos` loss (no reparam needed — `(sinθ,cosθ)` was a misread in the original plan); its gradient was simply **swamped by the heatmap-CE weight (6.0)**. Lever = **up-weight the aim term** (Exp #3 sweeps `player_aim` ∈ {5,10}) + **360 resolution** for the tiny turret cue. The question Exp #3 answers: does loss-balance alone reach ≤10°, or is a dedicated angular head needed?
...
- **The real test:** re-run the production 12-epoch learn-check and confirm it beats the **actual** stuck-at-5.5 baseline. **✅ Confirmed** (§10.2 / §6, 2026-06-28) — the gap was the sampler, not the decoder.
...
**Question.** For the encoder-pretrain → frozen-embedding → RL track (leader `resnet × flatten @ 180`; weak axes aim ~41°, bullet-pos ~1.3 wu): (1) Is the DreamerV3 image encoder a viable *frozen*-encoder candidate? (2) What encoders do JEPA-based RL/control projects actually use, and is any viable for us?

### 11.1 Q1 — DreamerV3 encoder: **NOT viable**
- **Architecture:** a small 4-stage stride-2 conv (SiLU) down to a coarse spatial bottleneck (paper "until 4×4"; current repo `minres 4` → ~`[3,16]`), then flatten — i.e. the **same class as our resnet / cnn trunks**, no expected gain on aim/bullet-pos, the same resolution-bound limit we already hit. Kernel/norm differ by version (official JAX: k5 + RMSNorm; NM512 torch port: k4 + channel-wise LayerNorm) — not a differentiator.
...
1. **Our direction is field-standard, not a compromise** — TD-MPC2 and DreamerV3 both use small CNNs at 64–96px for control. Validates `resnet × flatten` + the #12 small-input axis.
...
- **Issues:** [#3](https://github.com/urciuolim/TankTwinStickShooter/issues/3) parent · [#5–8](https://github.com/urciuolim/TankTwinStickShooter/issues/5) phases · [#12](https://github.com/urciuolim/TankTwinStickShooter/issues/12) follow-up · [#15](https://github.com/urciuolim/TankTwinStickShooter/issues/15) experimental program · [#13](https://github.com/urciuolim/TankTwinStickShooter/issues/13) model store/card · [#14](https://github.com/urciuolim/TankTwinStickShooter/issues/14) reproducible dataset shuffle (landed) · [#16](https://github.com/urciuolim/TankTwinStickShooter/issues/16) config-file params
