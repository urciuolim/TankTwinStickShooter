...
| 3 | **Readout fixes on the leader** — aim up-weight × heatmap-sharpen × resolution, on `impala × flatten` | 🔄 In progress (2026-06-29) | Decloak the true metric ceiling: close aim/bullet-pos gaps + presence recalibration; 8-cell knob sweep running |
...
**Goal.** Decloak the true metric ceiling of the leader (`impala × flatten`) by fixing the three architecture-independent readout problems Sweep 1 (#1) exposed — aim (~41°), bullet-pos (~1.32 wu), presence (F1 ~0.39 @ thr-0). Sweep 1 showed these don't move with trunk/pooling, so they're attacked at the **readout/loss level** on the single leading config, *before* committing compute to a wider grid (§4 one-factor attribution — don't confound a readout fix with an architecture change).
...
**What changed vs Sweep 1 (Exp #1).** Same trunk/pooling/data lineage (`impala × flatten`, `decode-v2_reshuffled`, 12 epochs, batch 256, lr 3e-3, seed 0, `heatmap_weight=6.0`, CUDA). The deltas are the readout knobs added in build commit `c856005` (platform-gate GO, 111 tests):
...
*Baselines to beat — Sweep 1 leader `impala × flatten @ 180` (spatial): position **0.33 wu**, aim **41.3°**, bullet-pos **1.32 wu**, presence F1 **0.39** @thr0 (→ **~0.76** achievable by calibration, Exp #2).*
